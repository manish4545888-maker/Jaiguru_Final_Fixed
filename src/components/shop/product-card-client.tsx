...
/**
 * Client-safe product card for the /products grid. Visually identical to
 * the server ProductCard but consumes pre-computed geo-aware display
 * labels (built by /api/products/filter), so it can re-render instantly
 * on filter changes without a page reload.
 */
...
function FeatureBadge({ product }: { product: ProductCardPayload }) {
...
function ProductImagePlaceholder({ product }: { product: ProductCardPayload }) {
  return (
    <div
...
export function ProductCardClient({ product }: { product: ProductCardPayload }) {
  const hasDiscount =
  ...
  const effectiveLabel = product.displayEffectiveLabel ?? formatPrice(product.discountPrice ?? product.price);
  const originalLabel = hasDiscount
    ? (product.displayOriginalLabel ?? formatPrice(product.price))
    : null;
    ...
    <article className="glass-card-light group flex flex-col overflow-hidden rounded-[var(--jaiguru-product-card-radius)] transition-all duration-300 hover:-translate-y-1.5 hover:border-golden/70 hover:shadow-[0_18px_50px_rgba(250,204,21,0.28)]">
      ...
      <div className="flex flex-1 flex-col gap-2.5 p-5">
        ...
        <div className="mt-1 flex items-baseline gap-2">
          ...
          {originalLabel ? (
            <span className="text-sm text-slate-400 line-through">{originalLabel}</span>
          ) : null}
        </div>
        {product.displayNote ? (
          <p className="text-[10px] leading-snug text-slate-500">{product.displayNote}</p>
        ) : null}
